...
def get_matchup_stats(matchup, session_id=None, message=None):
    ...
    if session_id:
        for vote in matchup.votes:
            if vote.session_id == session_id:
                session_id_matchup_vote_is = vote.contestant_id
                break

    # Query all comments related to matchup.id
    comments_query = Comment.query.filter_by(matchup_id=matchup.id).order_by(Comment.timestamp.desc()).all()
    comments = [
        {
            'session_id': c.session_id,
            'text': c.text,
            'timestamp': c.timestamp.isoformat() if hasattr(c.timestamp, 'isoformat') else str(c.timestamp),
            'cool_name': build_session_ids_coolname(c.session_id),
            'time_ago': build_comment_time_ago(c.timestamp),
            'matchup_contestant_vote_a_or_b': build_session_ids_vote_color(matchup.id, c.session_id), 
        }
